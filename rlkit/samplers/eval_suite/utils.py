...
def create_regular_image_directories(save_folder, epoch):
    cnn_path = f"{save_folder}/epochs/{epoch}/cnn_images"
    cnn_color_path = f"{save_folder}/epochs/{epoch}/cnn_color_images"
    cnn_color_full_path = f"{save_folder}/epochs/{epoch}/cnn_color_full_images"
    corners_path = f"{save_folder}/epochs/{epoch}/corners_images"
    eval_path = f"{save_folder}/epochs/{epoch}/eval_images"
    try:
...
def save_regular_images(env, save_folder, epoch, step_number, aux_output):
    corner_image, eval_image, cnn_color_image_full, cnn_color_image, cnn_image = env.capture_images(aux_output)
    cv2.imwrite(f'{save_folder}/epochs/{epoch}/corners_images/{str(step_number).zfill(3)}.png', corner_image)
    cv2.imwrite(f'{save_folder}/epochs/{epoch}/eval_images/{str(step_number).zfill(3)}.png', eval_image)
    cv2.imwrite(f'{save_folder}/epochs/{epoch}/cnn_images/{str(step_number).zfill(3)}.png', cnn_image)
    cv2.imwrite(f'{save_folder}/epochs/{epoch}/cnn_color_images/{str(step_number).zfill(3)}.png', cnn_color_image)
    cv2.imwrite(f'{save_folder}/epochs/{epoch}/cnn_color_full_images/{str(step_number).zfill(3)}.png', cnn_color_image_full)